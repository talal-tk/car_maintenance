...
if (isset($_POST['submit'])) {
    ...
    // Handling issues (checkboxes)
    if (isset($_POST['issue'])) {
        $issues = $_POST['issue']; // Array of issues
        $all_issues = implode(', ', $issues); // Convert the array to a comma-separated string
    } else {
        $all_issues = "No issues selected"; // If no issues are selected
    }
...
<body>
    ...
    <header>
        ...
        <form action="loginf.php" method="POST">
            <h1 id="ttt">Car Maintenance Dashboard - <ins>Admin</ins> </h1>
    ...
    <aside>
        <nav>
            <ul>
                ...
                <li><a href="#AddEmployee"onclick="showSection('AddEmployee')">Add Employee</a></li>  <!--for manager -->
                <li><a href="#showEmployee"onclick="showSection('showEmployee')">show Employees</a></li>      <!--for manager -->
<hr>
                <li><a href="#postONX"onclick="showSection('postOnX')"> Post on X </a></li>
            </ul>
        </nav>
    </aside>
...
?>
    ...
    <a href="https://twitter.com/share?ref_src=twsrc%5Etfw" class="twitter-button" data-size="large" data-text="Hi im wroking on " data-related="Golden_Oil_KSA" data-lang="en" data-show-count="false">Tweet</a>
...
">Select File</label>
        ...
        <section id="showEmployee" class="content-section">
           ...
           if (mysqli_num_rows($result) > 0) {
                echo "<table> <tr> <th>Employee id </th> <th>Name </th> <th>Phone </th> <th>Email </th> <th>Username </th> <th>Type </th> </tr>";
             // output data of each row
             while($row = mysqli_fetch_assoc($result)) {
               echo "<tr> <td>" . $row["employee_id"]. "</td> <td>" . $row["name"]. "</td> <td>". $row["phone"].  "</td> <td>" . $row["email"]. "</td> <td>" . $row["username"]. "</td> <td>" . $row["employee_type"]. "</td> </tr>";
             } 
             echo "</table>";
           } else {
             echo "0 results";
